...
full join games on games.winner_id = teams.team_id 
where year = 2014 and round = 'Eighth-Final' 
UNION 
select name from teams 
full join games on games.opponent_id = teams.team_id 
where year = 2014 and round = 'Eighth-Final'
order by name")"

echo -e "\nList of unique winning team names in the whole data set:"
echo  "$($PSQL "select distinct(name) from teams join games on games.winner_id = teams.team_id order by name")"
